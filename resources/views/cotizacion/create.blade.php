                ...
                <div class="col-12">
                    <div class="mb-2">
                        ...
                        <div class="top-right-button-container">
                            ...
                            <button type="button" class="btn btn-primary btn-lg top-right-button mr-1" onclick="Submit_qoute()">Generar Cotización</button>
                        </div>
                                    ...
                                    <div class="modal-body">
                                        <form>
                                            ...
                                            <div class="form-group">
                                                ...
                                                </select><span class="select2 select2-container select2-container--bootstrap" dir="ltr" style="width: 100%;"><span class="selection"><span class="select2-selection select2-selection--multiple form-control" role="combobox" aria-haspopup="true" aria-expanded="false" tabindex="-1"><ul class="select2-selection__rendered"><li class="select2-search select2-search--inline"><input class="select2-search__field" type="search" tabindex="0" autocomplete="off" autocorrect="off" autocapitalize="none" spellcheck="false" role="textbox" aria-autocomplete="list" placeholder="" style="width: 0.75em;"></li></ul></span></span><span class="dropdown-wrapper" aria-hidden="true"></span></span>
                                            </div>
...
<script>
    ...
    $(document).ready( function() {
         document.getElementById("iterator_qoute").value=0;
         setBlankForms()
      } );
  ...
  $("#selectClientFull").click(function(){
      ...
      $.ajax({
              ...
              headers: {'X-CSRF-TOKEN': $('#add_qoute #_token').val()},
              type: 'GET',
              cache: false,
              data: guestSelected,
  ...
  $("#quote_provider_new_accept ").click(function(event){
      ...
      if(confirmacion){
          ...
          let iterator_init=$("#iterator_qoute").val();
          let iterator=parseInt(iterator_init);
          iterator+=1;

          $("#tbody_Qoute").append(
              $('<tr id="reg_'+iterator+'">').append(
                  $('<input type="hidden" id="num_provider_id_'+iterator+'" value='+$("#_provider").val()+'>'),
                  $('<td id="provider_'+iterator+'" >').css('text-align','center').append($("#providerName").val())
              ).append( 
                  $('<td id="marca_'+iterator+'">').css('text-align','center').append($("#marca").val())
              ).append( 
                  $('<td id="cantidad_'+iterator+'">').css('text-align','center').append($("#cantidad").val())
              ).append( 
                  $('<td id="referencia_'+iterator+'">').css('text-align','center').append($("#referencia").val())
              ).append( 
                  $('<td id="descripcion_'+iterator+'">').css('text-align','center').append($("#descripcion").val())
              ).append( 
                  $('<td id="peso_unit_lbs_'+iterator+'">').css('text-align','center').append($("#peso_unit_lbs").val())
              ).append( 
                  $('<td id="costo_unit_lbs_'+iterator+'">').css('text-align','center').append($("#costo_unit_lbs").val())
              ).append( 
                  $('<td id="costo_unit_cop_'+iterator+'">').css('text-align','center').append($("#costo_unit_cop").val())
              ).append( 
                  $('<td id="dias_entrega_'+iterator+'">').css('text-align','center').append($("#dias_entrega").val())
              ).append( 
                  $('<td>').css('text-align','center').append(
                      '<button type="button" onclick="drop('+iterator+')" class="btn btn-danger default"> <i class="glyph-icon simple-icon-trash"></i></button>'+
                      '<button type="button" onclick="edit('+iterator+')" class="btn btn-info default"> <i class="glyph-icon simple-icon-pencil"></i></button>'
                  )
              )
          );
...
////---------------------------------------------------
  ...
  $("#add_qoute").on("submit",function(){
      ...
      let ValoresCalculoEstablecidos=$("#_haveCalc").val();
          if(ValoresCalculoEstablecidos=="true"){
              ...
              setTimeout(() => {
                  $("#alert_val_calc").hide();}, 2000);
              return false;
          }
  });
  
  function establecerDatosCotizacion(array,cant,ref,desc,marca,dias,precioVentaUnitarioXItem,precioVentaTotalXItem,precioGranTotalVenta,notas_cotizacion_export){
  ...
  let pesoTotalXItem = [];
      ...
      console.log("pesoTotalXItem"+pesoTotalXItem);

      //
      //Suma Peso Total  
      // var @sumaPesoTotal
      let sumaPesoTotal=0;
      pesoTotalXItem.forEach(function(elemento, indice, array){
          sumaPesoTotal+=parseFloat(elemento);
      });
      console.log("sumaPesoTotal: "+sumaPesoTotal);
      //Fin Suma Peso Total 
      ...
      console.log("costoTotalXPeso: "+costoTotalXPeso);
      ...
      console.log("porcPesoXItem: "+porcPesoXItem);

      let  costoTotalAntePeso=[];
      array.forEach(function(elemento, indice, array) {
          costoTotalAntePeso[indice]=cant[indice]*cosl[indice];
      });
      console.log("costoTotalAntePeso: "+costoTotalAntePeso);
      ...
      console.log("costoValorImportXPeso: "+costoValorImportXPeso);

      let costoValFleteInternoIntUSD= [];
      array.forEach(function(elemento, indice, array) {
          costoValFleteInternoIntUSD[indice]=porcPesoXItem[indice]*parseFloat(flete_usd_calc);
      });
      console.log("costoValFleteInternoIntUSD: "+costoValFleteInternoIntUSD);
      ...
      console.log("CostoTotalPesosPartsImpor: "+CostoTotalPesosPartsImpor);
      ...
      console.log("costoTotalEnPesos: "+costoTotalEnPesos);
      ...
      costoTotalAntePeso.forEach(function(elemento, indice, array) {
          costoTotalPartes+=elemento;
      });

      let costoTotalPFN=0;
      CostoTotal.forEach(function(elemento, indice, array) {
      ...
      CostoTotalPesosPartsImpor.forEach(function(elemento, indice, array) {
          GranTotalEnPesosPartsImport+=elemento;
      });

      costoTotalEnPesos.forEach(function(elemento, indice, array) {
      ...
      console.log("costoTotalSinAdc: "+costoTotalSinAdc);
       //
      //Fin Totales 
      //
      ...
      console.log("CostoUnitPesosPartsImpor: "+CostoUnitPesosPartsImpor);
      ...
      console.log("PorcPesosAcuerdoPesos: "+PorcPesosAcuerdoPesos);
     ...
     Res_costoGranTotalPesosconAdc=(GranTotalEnPesosPartsImport+GranTotalCostoPesos)+parseFloat(flete_cop_calc)+parseFloat(nacionalizacion_calc)+parseFloat(transportadora_calc)+((parseFloat(flete_usd_calc)+parseFloat(comision_banco_transf_exterior_calc))*dolarRealCalc)
      console.log("Res_costoGranTotalPesosconAdc: "+Res_costoGranTotalPesosconAdc);
      ...
      console.log("costoTotalPesosImporMasNal: "+costoTotalPesosImporMasNal);
     
      //Precios de Venta

      //Venta Total
      let precioVentaTotalXItem=[];
      costoTotalPesosImporMasNal.forEach(function(elemento, indice, array) {
      ...
      console.log('precioVentaTotalXItem: '+precioVentaTotalXItem);

      //Venta Unitaria
      let precioVentaUnitarioXItem=[];
      
      precioVentaTotalXItem.forEach(function(elemento, indice, array) {
      ...
      console.log('precioVentaUnitarioXItem: '+precioVentaUnitarioXItem);
